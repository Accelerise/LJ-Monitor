...
def chengjiao_spider(db_cj,url_page=u"http://bj.lianjia.com/chengjiao/pg1rs%E5%86%A0%E5%BA%AD%E5%9B%AD"):
    ...
    for cj in cj_list:
        ...
        totalPriceDOM=cj.find('div',{'class':'totalPrice'})
        if totalPriceDOM:
            info_dict.update({u'签约总价': totalPriceDOM.text})
        dealDateDOM=cj.find('div',{'class':'dealDate'})
        if dealDateDOM:
            info_dict.update({u'签约时间':dealDateDOM.text})
        unitPriceDOM=cj.find('div',{'class':'unitPrice'})
        if unitPriceDOM:
            info_dict.update({u'签约单价': unitPriceDOM.text})
